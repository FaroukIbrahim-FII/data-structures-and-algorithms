...
const addValues = (arr) => {
  let newValueArr = arr.reduce((acc, item) => acc += item, 0);
  return newValueArr;
};
...
const addPurchases = (arr) => {
  let totalPrice = arr.reduce((acc, item) => acc += item.purchasePrice, 0);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let summ = arr.reduce((acc, item) => arr.includes(item) ? acc + 1 : acc + 0, 0);
  return summ;
};
...
const reversedString = (str) => {
  let Arr = [...str];
  let reverse = Arr.reduce((acc, item) => item + acc, '');
  return reverse;
};

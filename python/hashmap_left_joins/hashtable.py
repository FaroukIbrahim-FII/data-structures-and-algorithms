...
class HashTable:
    ...
    # using stackoverflow to make the hashmap itretable
    def __iter__(self):
        left_list = []
        for item in self.__buckets:
            if item :
                current = item.head
                left_list.append(current.val[0])
                left_list.append(current.val[1])
                yield left_list


    def custom_hash(self, key):
        return sum([ord(char) for char in key]) * 10 % self.__size
